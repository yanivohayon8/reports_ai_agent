...
from backend.agents.summary_agent.summary_dataset_synthesizer import (
    SummaryDatasetSynthesizer,
    save_summary_dataset,
)


def main():
    parser = argparse.ArgumentParser(description="Summary dataset synthesizer CLI")
    ...
    parser.add_argument("--config-path", type=str, default="backend/agents/summary_agent/summary_dataset_synthesizer_config.yaml")
    ...
    synth = SummaryDatasetSynthesizer(
        llm=llm,
    ...
    def process(pdf: Path):
        ...
        save_summary_dataset(rows, pdf, output_dir)
