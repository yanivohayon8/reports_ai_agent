...
from backend.agents.summary_agent.summary_dataset_synthesizer import load_summary_dataset
...
class SummaryDatasetEvaluator:
    def __init__(self, ground_truth_dataset_path: Path, llm, selected_metrics: List[str] | None = None):
        self.ground_truth_dataset = load_summary_dataset(ground_truth_dataset_path)
        self.metrics = None
        if selected_metrics:
    ...
    def _generate_dataset(self, agent: SummaryAgent):
        dataset = []
        for row in self.ground_truth_dataset:
            query = row.get("question")
            reference = row.get("answer")
            context_text = row.get("context_text", "")
            agent_response = agent.summarize_with_query(query, context_text)
            sample = {
                "user_input": query,
    ...
    def evaluate(self, agent: SummaryAgent):
        dataset = self._generate_dataset(agent)
        return ragas_evaluate(dataset, metrics=self.metrics, llm=self.evaluator_llm)

    def save_results(self, evaluation_result, output_path: Path, agent: SummaryAgent | None = None):
        try:
