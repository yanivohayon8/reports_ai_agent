...
class SummaryDatasetSynthesizer:
    def __init__(
        self,
        llm: BaseChatModel,
        min_chunk_size: int,
...
def save_summary_dataset(rows: List[dict], pdf_path: Path, output_dir: Path) -> int:
...
def load_summary_dataset(dataset_path: Path) -> List[dict]:
    rows: List[dict] = []
