...
from backend.agents.needle_agent.needle_agent_graph import NeedleAgentGraph
...
class NeedleEvaluatorGraph:
    """
    RAGAS evaluator specifically for NeedleAgentGraph (Graph RAG).
    """
    ...
    def evaluate(self, agent: NeedleAgentGraph):
        """
        Evaluate the NeedleAgentGraph using RAGAS metrics.
        """
        dataset = self._generate_dataset(agent)
        result = ragas_evaluate(dataset, metrics=self.metrics, llm=self.evaluator_llm)
        return result
    
    def _generate_dataset(self, agent: NeedleAgentGraph):
        """
        Generate evaluation dataset from agent responses.
        """
        dataset = []
    ...
    def save_results(self, evaluation_result: EvaluationResult, output_path: Path, agent: NeedleAgentGraph = None):
        """
        ...
        saved_results = {
            ...
            "agent_type": "NeedleAgentGraph (Graph RAG)"
        }
    ...
    def _compute_mean(self, df: DataFrame) -> dict:
        """
        Compute mean scores from evaluation results.
        """
        try:
